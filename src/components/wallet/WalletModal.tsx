...
export const WalletModal = ({ isOpen, onClose, onSelectWallet, isConnecting }: WalletModalProps) => {
    ...
    // avoid window checks during SSR — compute on client
    const [isClient, setIsClient] = useState(false);
    const [isMetaMaskInstalled, setIsMetaMaskInstalled] = useState(false);
    const [isCoinbaseInstalled, setIsCoinbaseInstalled] = useState(false);
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            ...
            if (providers && Array.isArray(providers)) {
                ...
                setIsMetaMaskInstalled(providers.some((p: any) => p.isMetaMask && !p.isCoinbaseWallet));
                setIsCoinbaseInstalled(providers.some((p: any) => p.isCoinbaseWallet));
                setIsTrustInstalled(providers.some((p: any) => p.isTrust || p.isTrustWallet));
                setHasInjectedProvider(true);
            } else if (eth) {
                // Single provider
                setIsMetaMaskInstalled(!!eth.isMetaMask);
                setIsCoinbaseInstalled(!!eth.isCoinbaseWallet);
                setIsTrustInstalled(!!(eth.isTrust || eth.isTrustWallet));
                setHasInjectedProvider(true);
            } else {
    ...
    const walletOptions: WalletOption[] = [
        {
            ...
            name: 'MetaMask',
            icon: <MetaMaskIcon />,
            description: isClient ? (isMetaMaskInstalled ? 'Popular browser extension' : 'Install to connect') : 'Detecting…',
        ...
        },
        {
            ...
            icon: <CoinbaseIcon />,
            description: isClient ? (isCoinbaseInstalled ? 'Coinbase browser extension' : 'Install to connect') : 'Detecting…',
        ...
        },
        {
            ...
            description: isClient ? (isTrustInstalled ? 'Trust browser extension' : 'Use WalletConnect') : 'Detecting…',
        ...
        },
        {
            ...
            name: 'Browser Wallet',
            icon: <InjectedIcon />,
            description: isClient ? (hasInjectedProvider ? 'Detected wallet extension' : 'No wallet found') : 'Detecting…',
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Connect Wallet" maxWidth="max-w-lg">
            <div className="space-y-4">
                {/* Subtitle */}
                <p className="text-text-secondary text-sm">
                    Connect your wallet to access your portfolio and start investing.
                </p>
                ...
                <div className="space-y-2">
                    {walletOptions.map((wallet) => {
                        ...
                        return (
                            ...
                            >
                                {/* Icon */}
                                <div className="flex-shrink-0 w-11 h-11 rounded-xl bg-white/5 flex items-center justify-center overflow-hidden">
                                    {wallet.icon}
                                </div>
                                ...
                                <div className="flex-1 text-left">
                                    ...
                                    <p className="text-text-muted text-xs mt-0.5">{wallet.description}</p>
                                </div>
                                ...
                                <div className="flex-shrink-0">
                                    {isLoading ? (
                                        ...
                                        <span className="text-xs text-primary-400 font-medium">Install</span>
                                    )}
                                </div>
                            </motion.button>
                        );

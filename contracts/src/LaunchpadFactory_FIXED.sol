...
/**
 * @title LaunchpadFactory (SECURITY HARDENED VERSION)
 ...
 * - Added ERC20 token validation
 * - Added pool creation fee
 * - Added maximum pools per creator limit
 * - Added token supply verification
 * - Added duplicate pool detection
 * - Added event emissions
 */
contract LaunchpadFactory is Ownable, ReentrancyGuard {
    // ============ Constants ============

    /// @notice Maximum pools one creator can make
    uint256 public constant MAX_POOLS_PER_CREATOR = 10;

    /// @notice Minimum token price to prevent zero-price attacks
    uint256 public constant MIN_TOKEN_PRICE = 1e12; // 0.000001 ETH

    /// @notice Maximum token price to prevent overflow
    uint256 public constant MAX_TOKEN_PRICE = 1e24; // 1,000,000 ETH
    ...
    /// @notice Mapping to detect duplicate pools
    mapping(bytes32 => address) public poolHashes;
    ...
    /// @notice Pool creation fee (in wei)
    ...
    constructor(
        ...
        maxDuration = 30 days;
        creationFee = 0.01 ether; // FIX: Add creation fee to prevent spam
    }
     ...
     * 
     * SECURITY IMPROVEMENTS:
     * - FIX: Validates token price is non-zero
     * - FIX: Validates ERC20 token interface
     * - FIX: Checks token supply
     * - FIX: Enforces max pools per creator
     * - FIX: Requires creation fee payment
     * - FIX: Prevents duplicate pools
     */
    function createPool(
        ...
        // FIX: Check creation fee
        if (msg.value < creationFee) revert InsufficientCreationFee();

        // FIX: Check max pools per creator
        if (creatorPools[msg.sender].length >= MAX_POOLS_PER_CREATOR) {
        ...
        // FIX: Validate token price range
        if (tokenPrice < MIN_TOKEN_PRICE || tokenPrice > MAX_TOKEN_PRICE) {
            revert InvalidTokenPrice();
        }

        // FIX: Validate ERC20 token
        try IERC20(saleToken).totalSupply() returns (uint256 totalSupply) {
            // Calculate required supply
            uint256 requiredSupply = (hardCap * 1e18) / tokenPrice;
            
            // FIX: Ensure token has sufficient supply
            if (totalSupply < requiredSupply) revert InsufficientTokenSupply();
        } catch {
            revert InvalidToken();
        }
        ...
        if (poolHashes[poolHash] != address(0)) revert DuplicatePool();
        ...
        // Store pool hash to prevent duplicates
        poolHashes[poolHash] = poolAddress;
